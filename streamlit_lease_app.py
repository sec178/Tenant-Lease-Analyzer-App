...
if 'show_manual_override' not in st.session_state:
...
def main():
        ...
        if uploaded_file is not None:
            ...
            if not st.session_state.lease_loaded:
                ...
                if load_result['status'] == 'success':
                    st.session_state.pdf_metadata = load_result.get('metadata', {})
                    ...
                    for field, display_name in critical_fields.items():
                        if not st.session_state.pdf_metadata.get(field):
                            missing_fields.append(display_name)
                    
                    if missing_fields:
                        st.warning(f"⚠️ PDF loaded but some information was not found: {', '.join(missing_fields)}")
                        st.info("👇 You can fill in the missing information below to get a complete analysis.")
                    ...
                    display_metadata(st.session_state.pdf_metadata)
            ...
            # Show manual override form if needed
            if st.session_state.show_manual_override and not st.session_state.lease_loaded:
                st.divider()
                st.markdown("### 📝 Fill in Missing Information")
                st.info("The fields below are pre-filled with any information extracted from your PDF. Please fill in or correct any missing/incorrect information.")
                
                with st.form("pdf_override_form"):
                    ...
                    with col1:
                        property_address = st.text_input(
                            "Property Address", 
                            value=metadata.get('property_address', '') or '',
                            help="Full street address"
                        )
                        city = st.text_input(
                            "City *", 
                            value=metadata.get('city', '') or '',
                            help="City where property is located (required)"
                        )
                        state = st.text_input(
                            "State *", 
                            value=metadata.get('state', '') or '',
                            help="State (e.g., CA, NY, TX) (required)"
                        )
                    
                    with col2:
                        zip_code = st.text_input(
                            "ZIP Code", 
                            value=metadata.get('zip_code', '') or '',
                            help="5-digit ZIP code"
                        )
                        landlord_name = st.text_input(
                            "Landlord/Property Manager Name", 
                            value=metadata.get('landlord_name', '') or '',
                            help="Name of landlord or company"
                        )
                    ...
                    with col3:
                        monthly_rent_default = metadata.get('monthly_rent', 0) or 0
                        monthly_rent = st.number_input(
                            "Monthly Rent ($)", 
                            min_value=0.0, 
                            step=100.0, 
                            value=float(monthly_rent_default),
                            help="Monthly rent amount"
                        )
                        security_deposit_default = metadata.get('security_deposit', 0) or 0
                        security_deposit = st.number_input(
                            "Security Deposit ($)", 
                            min_value=0.0, 
                            step=100.0, 
                            value=float(security_deposit_default),
                            help="Security deposit amount"
                        )
                    
                    with col4:
                        bedrooms_default = metadata.get('number_of_bedrooms', 1) or 1
                        number_of_bedrooms = st.number_input(
                            "Number of Bedrooms", 
                            min_value=0, 
                            max_value=10, 
                            step=1, 
                            value=int(bedrooms_default)
                        )
                        bathrooms_default = metadata.get('number_of_bathrooms', 1.0) or 1.0
                        number_of_bathrooms = st.number_input(
                            "Number of Bathrooms", 
                            min_value=0.0, 
                            max_value=10.0, 
                            step=0.5, 
                            value=float(bathrooms_default)
                        )
                    ...
                    with col5:
                        start_default = metadata.get('lease_start_date')
                        if start_default and start_default != 'N/A':
                            try:
                                from datetime import datetime as dt
                                lease_start_date = st.date_input(
                                    "Lease Start Date",
                                    value=dt.strptime(str(start_default), '%Y-%m-%d').date(),
                                    help="Date lease begins"
                                )
                            except:
                    ...
                    with col6:
                        end_default = metadata.get('lease_end_date')
                        if end_default and end_default != 'N/A':
                            try:
                                from datetime import datetime as dt
                                lease_end_date = st.date_input(
                                    "Lease End Date",
                                    value=dt.strptime(str(end_default), '%Y-%m-%d').date(),
                                    help="Date lease ends"
                                )
                            except:
                    ...
                    submitted = st.form_submit_button("✅ Confirm Information", type="primary")
                    
                    if submitted:
                        if not city or not state:
                            st.error("⚠️ City and State are required.")
                        else:
                            # Update metadata with corrected/completed information
                            updated_metadata = {
                            ...
                            # Update analyzer metadata (lease_text is already loaded from PDF)
                            analyzer.metadata = updated_metadata
                            st.session_state.lease_loaded = True
                            st.success("✅ Information confirmed! Ready for analysis.")
                            st.rerun()
    ...
    # -------------------------------
        ...
        st.caption("You can upload PDFs or images of your lease, photos of clauses, etc. for reference.")
        
        uploaded_files = st.file_uploader(
            "Upload supporting documents (optional)",
            type=['pdf', 'png', 'jpg', 'jpeg'],
            accept_multiple_files=True,
            help="Upload any documents or images related to your lease"
        )
                ...
                if 'pdf' in file_type:
                    ...
                    try:
                        with st.spinner(f"Extracting text from {uploaded_file.name}..."):
                            extracted_text = analyzer.analyzer.extract_lease_text(tmp_file_path)
                    ...
                    st.info(f"📷 Image uploaded: {uploaded_file.name}. Note: OCR not currently supported for images.")
            
            if supporting_text:
                with st.expander("📄 View Extracted Text from Uploaded Files"):
                    st.text_area("Extracted Text", supporting_text, height=200, disabled=True, key="extracted_text_view")
        ...
        with st.form("manual_lease_form"):
            ...
            with col1:
                ...
                city = st.text_input("City *", help="City where property is located (required)")
                state = st.text_input("State *", help="State (e.g., CA, NY, TX) (required)")
            ...
            if submitted:
                ...
                if not city or not state:
                    ...
                    # Create lease text combining manual entry and uploaded files
                    ...
                    # Add supporting text from uploaded files
                    if supporting_text:
                        lease_text += supporting_text
...
def display_full_results(results):
    ...
    with tabs[5]:
        ...
        st.text_area("Full Report", report, height=400, key="full_report_display")
